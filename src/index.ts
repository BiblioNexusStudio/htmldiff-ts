...
export class HtmlDiff extends AbstractDiff {
  ...
  protected replaceIsolatedDiffTags() {
    this.oldIsolatedDiffTags = this.createIsolatedDiffTagPlaceholders(
      this.oldWords,
    );
    this.newIsolatedDiffTags = this.createIsolatedDiffTagPlaceholders(
      this.newWords,
    );
  }
  ...
  protected createIsolatedDiffTagPlaceholders(words: string[]): {
    ...
    for (let index = 0; index < words.length; index++) {
      ...
      const openIsolatedDiffTag = this.isOpeningIsolatedDiffTag(
        word,
      ...
      if (openIsolatedDiffTag) {
        if (
          this.isSelfClosingTag(word) ||
          word.toLowerCase().includes("<img")
        ) {
        ...
        this.isClosingIsolatedDiffTag(word, currentIsolatedDiffTag)
      ) {
        openIsolatedDiffTags--;
        if (openIsolatedDiffTags === 0) {
          isolatedDiffTagIndices.push({
    ...
    for (const isolatedDiffTagIndex of isolatedDiffTagIndices) {
      ...
      const placeholderString = this.config.getIsolatedDiffTagPlaceholder(
        isolatedDiffTagIndex.tagType,
      );
      isolatedDiffTagScript[start] = words.splice(
        start,
        isolatedDiffTagIndex.length,
        placeholderString,
      );
  ...
  protected isOpeningIsolatedDiffTag(
    item: string,
    currentIsolatedDiffTag: string | null = null,
  ): string | false {
    const tagsToMatch =
      currentIsolatedDiffTag !== null
        ? {
            [currentIsolatedDiffTag]: this.config.getIsolatedDiffTagPlaceholder(
              currentIsolatedDiffTag,
            ),
          }
  ...
  protected processInsertOperation(
    operation: Operation,
    cssClass: string,
  ): void {
    const text: string[] = [];
    for (let pos = operation.startInNew; pos < operation.endInNew; pos++) {
      ...
      if (
        this.config.isIsolatedDiffTagPlaceholder(s) &&
        this.newIsolatedDiffTags[pos]
      ) {
  ...
  protected processDeleteOperation(
    operation: Operation,
    cssClass: string,
  ): void {
    const text: string[] = [];
  ...
  protected diffElements(
    ...
    const diff = (this.constructor as typeof HtmlDiff).create(
      oldText,
      newText,
      this.config,
    );
  ...
  protected diffElementsByAttribute(
    oldText: string,
    newText: string,
    attribute: string,
    element: string,
  ...
  protected findIsolatedDiffTagsInOld(
    operation: Operation,
    posInNew: number,
  ): string[] {
  ...
  protected insertTag(tag: string, cssClass: string, words: string[]): void {
    while (words.length > 0) {
      ...
      if (
        workTag[0] &&
        this.isOpeningTag(workTag[0]) &&
        !this.isClosingTag(workTag[0])
      ) {
  ...
  protected extractConsecutiveWords(
    words: string[],
    condition: string,
  ): string[] {
    ...
    for (let indexInOld = startInOld; indexInOld < endInOld; indexInOld++) {
      ...
      for (const indexInNew of this.wordIndices[index]) {
        ...
        if (
          ...
          (groupDiffs &&
            bestMatchSize > 0 &&
            this.oldTextIsOnlyWhitespace(bestMatchInOld, bestMatchSize))
        ) {
  ...
  protected oldTextIsOnlyWhitespace(
    startingAtWord: number,
    wordCount: number,
  ): boolean {
    let isWhitespace = true;

    for (
      let index = startingAtWord;
      index < startingAtWord + wordCount;
      index++
    ) {
